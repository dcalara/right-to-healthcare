...
const viridisColor = [[ 0.26700401,  0.00487433,  0.32941519],
...
function getGlobePolyColor(inputNum, minNum, maxNum) {
    ...
    let num = 128;    // default value for max = min (would cause divide by zero)
    if (maxNum != minNum) {
      num = Math.floor(255.99 * (inputNum - minNum) / (maxNum - minNum));
    }
    if (num < 0) {
      num = 0
    }
    ...
    const r = Math.floor(255 * viridisColor[num][0]);
    const g = Math.floor(255 * viridisColor[num][1]);
    const b = Math.floor(255 * viridisColor[num][2]);
...
d3.select("#selDataset1").on("change", function () {
    ...
    d3.json(req_url).then( function (dataByYear) {
        ...
        countryData.features.forEach( function (feature) {
            ...
            if(dataValue) {
                ...
                // Handle multiple polygons per country as folows:
                // Perhaps for recognition, multi-polygons are wrapped in an extra array of length 1,
                // but the polygons themselves must be arrays of length > 1.  So, if we see an
                // array of length 1, we need to use item [0]
                coordList.forEach( function (polygonInList) {

                    let correctedList = [];
                    if (polygonInList.length > 1) {
                        listToUse = polygonInList;
                    }
                    if (polygonInList.length == 1) {
                        listToUse = polygonInList[0];
                    }
                    listToUse.forEach( function(lonlat) {
                        if (lonlat.length == 2) { 
                          let latlon = [lonlat[1], lonlat[0]];
                          correctedList.push(latlon);
                        }
                    });

                    tempPoly = WE.polygon(correctedList, {
                        color: innerColor,
                        opacity: 0.9,
    ...
    }); // d3.json.then async for app data

}); // event listener for variable change
// Sorry for the pyramid of doom!!!
